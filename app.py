...
# --- REAL USER COUNT LOGIC (FILE-BASED) ---
def get_active_users():
    ...
    # Create/Update heartbeat file
    session_file = session_dir / f"{st.session_state.user_id}.lock"
    session_file.touch()
    
    # Cleanup files older than 30 seconds
...
st.markdown("""
    ...
    <div id="popup-alert">⚠️ POPUP BLOCKED! Enable popups to play.</div>
...
if st.session_state.stealth_mode:
    ...
    with tab1:
        ...
        if os.path.exists(game_dir):
            ...
            cols = st.columns(3)
            for i, f_name in enumerate(display):
                with cols[i % 3]:
                    with st.container(border=True):
                        st.subheader(f_name.replace(".html", "").replace("_", " ").title())
                        if st.button("PLAY", key=f"p_{f_name}"): launch_game(os.path.join(game_dir, f_name))
    with tab2:
