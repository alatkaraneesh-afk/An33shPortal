...
def get_active_users():
    ...
    # Clean up old files (> 20 seconds to give autorefresh breathing room)
    ...
    # Unique files remaining represent actual tabs open
...
if st.session_state.stealth_mode:
    st.set_page_config(
        page_title="Advanced Calculus - Module 4", page_icon="📝", layout="wide"
    )
else:
    st.set_page_config(
        page_title="AN33SH PORTAL", page_icon="🐦%E2%80%8D🔥", layout="wide"
    )
...
with st.sidebar:
    if st.session_state.stealth_mode:
        ...
        st.markdown(
            f'<div class="user-badge">● {active_now} USERS ONLINE</div>',
            unsafe_allow_html=True,
        )
        ...
        st.write("---")
        st.markdown(
            '<div class="spy-warning">IF YOU SUSPECT A TEACHER IS SPYING, PRESS THE BUTTON ON THE BOTTOM OR PRESS ALT+TAB.</div>',
            unsafe_allow_html=True,
        )
        ...
        st.write("---")
        st.markdown(
            '<a href="https://google.com" target="_self"><button style="width:100%; background:red; color:white; border-radius:10px; border:none; padding:12px; font-weight:bold;">⚠️ EMERGENCY EXIT</button></a>',
            unsafe_allow_html=True,
        )
...
if st.session_state.stealth_mode:
    ...
    st.info(
        "Current Topic: Linear Second-Order Equations with Constant Coefficients"
    )
    st.text_area("Notes", "Enter research data here...", height=400)
else:
    # Keeps background updates running safely every 5 seconds
    st_autorefresh(interval=5000, key="frequent_refresh")

    st.markdown('<div style="text-align: center;">', unsafe_allow_html=True)
    if os.path.exists(LOGO_PATH):
        st.image(LOGO_PATH, width=150)
    st.title("AN33SH PORTAL Bird🔥")

    # Fixed fragment counter behavior to stop randomized flicker crashes
    ...
    st.caption(
        "EMERGENCY SITES: https://docs.google.com/document/d/1xNdJqxSORoJ5iVOjP1ofgigySlX_q4jQazalNOpxwgU/edit?usp=sharing"
    )
    ...
    with tab1:
        if os.path.exists(GAME_DIR):
            all_games = sorted(
                [f for f in os.listdir(GAME_DIR) if f.endswith(".html")]
            )
            c1, c2 = st.columns(2)
            with c1:
                query = st.text_input(
                ...
                filtered = (
                    [f for f in all_games if query in f.lower()]
                    if query
                    else all_games
                )
    ...
    with tab2:
        st.markdown(
            '<div style="border:1px dashed #444;padding:50px;text-align:center;border-radius:20px;background:rgba(255,255,255,0.02);"><h2 style="color:#ff4b4b;">🛰️ STEALTH PROXY</h2><p>COMING SOON</p></div>',
            unsafe_allow_html=True,
        )
                    ...
                    with cols[i % 3]:
                        ...
                        if st.button(f"🎵 {s_label}", key=f"s_{s_file}"):
                            ...
                            st.components.v1.html(
                                f'<audio autoplay><source src="data:audio/mp3;base64,{s_b64}" type="audio/mp3"></audio>',
                                height=0,
                    )
        else:
            os.makedirs(SOUND_DIR, exist_ok=True)
            st.info(
                "Created sound folder. Add .mp3 files to static/sounds/ and refresh."
            )
